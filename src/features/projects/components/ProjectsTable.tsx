...
const columns:ColumnDef<Project,unknown>[]=[{accessorKey:"projectNumber",header:"Project",cell:({row})=><div><Link className="record-link" to={`/projects/${row.original.id}`}><strong>{row.original.projectNumber}</strong></Link><div className="muted">{row.original.name}</div></div>},{accessorKey:"client",header:"Client"},{accessorKey:"manager",header:"Manager"},{accessorKey:"progress",header:"Progress",cell:({row})=><div className="table-progress"><Progress value={row.original.progress} label=""/></div>},{accessorKey:"status",header:"Status",cell:({row})=><StatusBadge tone={getStatusTone(row.original.status)}>{humanize(row.original.status)}</StatusBadge>},{accessorKey:"plannedCompletion",header:"Completion"}];
